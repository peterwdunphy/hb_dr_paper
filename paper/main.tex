...
vision-threatening disease among people with DR (odds ratio 1.036, 95\% CI 1.023
to 1.049; $p < 0.001$, cluster-robust), corresponding to approximately 0.43 percentage points
per standard deviation. The gradient was consistent across all six race-by-age
strata examined ($\beta$ 0.032 to 0.046). Among adults aged 65 to 84, who are
near-universally Medicare eligible, the gradient was detectably but only
modestly shallower than among adults aged 40 to 64, an attenuation of about
12\% (interaction $-0.0036$, $p = 0.002$). The association persisted under a Leroux
...
post-stratification rather than to local variation. Decomposition shows this is
almost entirely \emph{racial} composition: holding age fixed leaves the
coefficient unchanged (0.150), whereas holding race fixed collapses it (0.034).
...
demographic composition, is only marginally narrowed by Medicare eligibility and
is not explained by direct measures of insurance coverage or diagnostic
intensity, and is distinct
...
Recent work has begun to address disease severity directly. Xie and colleagues,
studying 73,618 patients with diabetes at two academic centers, found that
residents of more deprived neighborhoods had substantially higher odds of
proliferative diabetic retinopathy, with a clear dose-response across quartiles
of the Area Deprivation Index.\citep{xie2025} That study establishes the
gradient in a large clinical population. Three questions it could not address
motivate the present analysis. Its sample consists of patients attending
tertiary referral centers, so its generalizability to the national population is
unknown, as the authors note. Its outcome is proliferative disease among all
patients with diabetes, which combines whether retinopathy developed at all with
how far it progressed. And, as the authors also note, it could not adjust for
neighborhood diabetes prevalence, leaving open whether the gradient reflects
more diabetes or worse control of it.

The unresolved question is therefore whether social disadvantage is associated
with how far disease has advanced \emph{among people who already have it},
in the general population rather than among referral-center attendees. That quantity,
...
across 3,111 counties. Eight of the 3,119 counties in the analytic sample have
populations too small for VEHSS to publish race-stratified estimates and
therefore contribute no cells; they are retained in county-level descriptive
analyses only.

Cell coverage is 13,035 of a possible 18,666. Missingness is driven by the size
of the demographic group rather than by any property of the outcome: VEHSS does
not publish an estimate where the county's population in that race-by-age cell
is very small. Among counties with no non-Hispanic Black estimate for ages 65 to
84, the median non-Hispanic Black population is 105, against 5,576 in counties
where the estimate is published, and 95\% have fewer than 1,000. The
corresponding medians for Hispanic adults are 420 against 6,128. The Black and
Hispanic strata are therefore estimated on counties with substantial minority
populations, which skew urban, and stratum-specific estimates should be read
with that in mind. Models include stratum fixed effects, so the social
...
Because the analytic file stacks up to six race-by-age cells per county, and
every county-level covariate is repeated across a county's cells, the
quasibinomial dispersion parameter alone is not an adequate treatment of the
resulting dependence: it rescales all standard errors by a single scalar
without modeling the within-county correlation, and the effective sample size
for a county-level covariate is nearer the 3,111 counties than the 13,035
cells. All standard errors and confidence intervals for county-level covariates
are therefore cluster-robust, clustered on county. This inflates them roughly
fourfold relative to the model-based values and is the basis for every interval
reported below. Continuous predictors were standardized, so coefficients
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/figB4_map.pdf}
\caption{Geographic distribution of the vision-threatening share of diagnosed
diabetic retinopathy. This is a case-mix measure and is nearly uncorrelated with
diabetic retinopathy prevalence itself (Spearman $\rho = -0.08$).}
\label{fig:map}
\end{figure}
...
The artifact is specifically a \emph{racial} composition effect, not an age
effect. Holding age fixed while allowing racial composition to vary leaves the
coefficient essentially unchanged at 0.150; holding race fixed while allowing
age to vary collapses it to 0.034, which is already the fully stratified value.
This is what the post-stratification mechanism predicts, since national
race-specific vision-threatening rates differ substantially while the
county-level social gradient is largely orthogonal to age composition.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/figB1_artifact.pdf}
\caption{The social vulnerability coefficient before and after holding county
demographic composition fixed.}
\label{fig:artifact}
\end{figure}
...
an odds ratio of 1.036 (95\% CI 1.023 to 1.049; $p<0.001$, with standard errors
clustered on county), equivalent to
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/figB2_strata.pdf}
\caption{Social vulnerability coefficient estimated separately within each
race-by-age stratum.}
\label{fig:strata}
\end{figure}
...
Medicare coverage is near-universal. It was narrower, but only slightly.
Estimated within demographic strata, the SVI coefficient was 0.037 among adults
aged 40 to 64 and 0.033 among adults aged 65 to 84. Because the two age bands
are drawn from the same counties, the correct test is an interaction within a
single model rather than a comparison of separately fitted coefficients; that
interaction is $-0.0036$ ($p = 0.002$, cluster-robust), an attenuation of
approximately 12\%. Medicare eligibility is thus associated with a detectably
but modestly shallower gradient, leaving roughly seven eighths of it intact. The
same pattern held in each race stratum separately (Table~\ref{tab:strata}).
...
Two covariates that appeared significant under model-based standard errors do
not survive clustering: ophthalmologist density ($p = 0.36$) and optometrist
density ($p = 0.19$). Neither is a study exposure, but the change is worth
noting, since it means eye-care provider density has no detectable independent
association with disease severity once within-county dependence is accounted
for. Rurality and population density remain associated
(Table~\ref{tab:main}).

\subsection{Sensitivity to the racial component of SVI and to provider density}

Because the overall SVI contains a racial and ethnic minority status theme, we
refitted the primary model using a composite of the remaining three themes,
constructed as the unweighted arithmetic mean of the county percentile rankings
for Theme 1 (socioeconomic status), Theme 2 (household characteristics), and
Theme 4 (housing type and transportation), that is \texttt{RPL\_THEME1},
\texttt{RPL\_THEME2} and \texttt{RPL\_THEME4}, then standardized to mean zero and
unit variance in the same way as the overall index. It correlates with the
overall SVI at 0.98. The
association persisted at 0.027 (95\% CI 0.015 to 0.040, $p<0.001$), about
three quarters of the overall-SVI estimate. Entered alone, the socioeconomic
status theme gave 0.034 (95\% CI 0.021 to 0.047).

The minority status theme was nonetheless the strongest single component,
whether entered alone (0.069) or jointly with the other three (0.071). We
emphasize that this is not the post-stratification artifact, which within-stratum
estimation removes by construction; it is a residual county-level association
between racial composition and disease severity within demographic strata, and
is consistent with the effects of residential segregation on care delivery
rather than with a compositional artifact.

Because VEHSS already adjusted its county random effects for ophthalmologists
per capita, our adjustment for ophthalmologist and optometrist density is
partly a second pass at the same factor. Removing those two terms moved the SVI
coefficient from 0.035 to 0.038, a change of 7\%, so the double adjustment is
not materially affecting the result.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/figB3_mechanism.pdf}
\caption{Social vulnerability coefficient after adding each set of external
covariates on a common sample. Insurance coverage and diagnostic volume leave
the gradient intact; preventive-screening completion absorbs roughly two fifths
of it.}
\label{fig:mechanism}
\end{figure}
...
(SVI $\times$ expansion $\times$ age band, $\beta = 0.002$, $p = 0.47$;
...
retinopathy, 13,035 county $\times$ race $\times$ age cells across 3,111
...
\begin{tabular}{lrrrl}
\toprule
Variable & $\beta$ & Cluster SE & $p$ & 95\% CI \\
\midrule
Social Vulnerability Index             & $0.0353$  & 0.0062 & $<0.001$ & $0.023$ to $0.048$ \\
Drive time to residency program (log)  & $-0.0174$ & 0.0053 & $0.001$  & $-0.028$ to $-0.007$ \\
Drive time to any ophthalmologist (log)& $-0.0018$ & 0.0071 & $0.80$   & $-0.016$ to $0.012$ \\
Ophthalmologists per 100,000           & $-0.0072$ & 0.0078 & $0.36$   & $-0.022$ to $0.008$ \\
Optometrists per 100,000               & $-0.0113$ & 0.0085 & $0.19$   & $-0.028$ to $0.005$ \\
Population density (log)               & $-0.0405$ & 0.0117 & $<0.001$ & $-0.063$ to $-0.018$ \\
Rural-Urban Continuum Code             & $-0.0584$ & 0.0107 & $<0.001$ & $-0.079$ to $-0.037$ \\
\midrule
\multicolumn{5}{l}{\footnotesize Stratum fixed effects included. Dispersion
parameter 1.93. Standard errors are cluster-robust,}\\
\multicolumn{5}{l}{\footnotesize clustered on county (3,111 clusters); model-based
SEs would be roughly four times smaller.}\\
\multicolumn{5}{l}{\footnotesize Unstratified comparison estimate for SVI:
$0.146$.}\\
\bottomrule
\end{tabular}
\end{table}
...
insurance coverage. Each block is estimated on its own complete-case sample
(13,035, 13,008 and 13,024 cells respectively), so each block carries its own
base row; all three round to the same value.}
\label{tab:external}
\begin{tabular}{llrrr}
\toprule
Domain & Specification & $\beta_{\mathrm{SVI}}$ & Cluster SE & $p$ \\
\midrule
\multirow{2}{*}{Diagnostic intensity}
 & Base (block sample)                & 0.0353 & 0.0062 & $<0.001$ \\
 & + Medicare imaging, E\&M, tests per 1,000 & 0.0354 & 0.0063 & $<0.001$ \\
\midrule
\multirow{3}{*}{Preventive screening}
 & Base (block sample)                & 0.0353 & 0.0062 & $<0.001$ \\
 & + Mammography screening            & 0.0206 & 0.0069 & $0.003$ \\
 & + Mammography and influenza vaccination & 0.0217 & 0.0077 & $0.005$ \\
\midrule
\multirow{3}{*}{Coverage}
 & Base (block sample)                & 0.0353 & 0.0062 & $<0.001$ \\
 & + Uninsured rate                   & 0.0335 & 0.0076 & $<0.001$ \\
 & + Uninsured rate and dual-eligible share & 0.0343 & 0.0091 & $<0.001$ \\
\midrule
\multicolumn{5}{l}{\footnotesize Standard errors cluster-robust, clustered on county.}\\
\bottomrule
\end{tabular}
\end{table}
%-------------------------------------------------------------------------
...
\toprule
Age band & Expansion status & Cells & Counties & $\beta_{\mathrm{SVI}}$ & Cluster SE \\
\midrule
40--64 years & Non-expansion & 2,736 & 1,127 & 0.0163 & 0.0125 \\
40--64 years & Expansion     & 3,822 & 1,780 & 0.0461 & 0.0065 \\
65--84 years & Non-expansion & 2,436 & 1,127 & 0.0122 & 0.0118 \\
65--84 years & Expansion     & 3,307 & 1,788 & 0.0421 & 0.0060 \\
\midrule
\multicolumn{6}{l}{\footnotesize Triple interaction SVI $\times$ expansion
$\times$ age band: $\beta = 0.0023$, cluster-robust $p = 0.47$.}\\
\multicolumn{6}{l}{\footnotesize Standard errors cluster-robust, clustered on county.}\\
\bottomrule
\end{tabular}
\end{table}
%-------------------------------------------------------------------------
...
Our findings both corroborate and refine the clinic-based evidence. Xie et al.
reported a dose-response between neighborhood deprivation and proliferative
disease among referral-center patients.\citep{xie2025} We reproduce a social
gradient in disease severity at national population scale, which is meaningful
corroboration given the different sampling frame. We depart from that work in
three respects. Conditioning on the presence of diabetic retinopathy separates
progression from occurrence, and we find the gradient in progression to be
several times that in occurrence among people with diabetes, so the two are not
interchangeable. Using the diabetes population as the explicit denominator
addresses the confounding by neighborhood diabetes prevalence that the earlier
study identified as an open limitation. And whereas that study found greater
distance from the ophthalmology clinic associated with proliferative disease, we
find no population-level counterpart; we take up that divergence, and the
referral selection that most plausibly explains it, in a companion analysis of
academic ophthalmology proximity using the same data.
...
First, coverage of the race-stratified cells is a function of demographic group
size, so the Black and Hispanic stratum estimates rest on a more urban subset of
counties than the non-Hispanic White estimates.

Second, this is an ecological analysis of county aggregates; associations cannot
...
Third, and most importantly, the outcome is derived from modeled estimates
...
Fourth, VEHSS county random effects were themselves estimated controlling for
...
Fifth, drive times model private vehicle travel on the road network. They
...
Sixth, indicator vintages are not perfectly aligned. DR estimates are for 2021,
...
Seventh, Puerto Rico is absent from the VEHSS source and is therefore unrepresented
