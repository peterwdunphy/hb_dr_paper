...
\title{\textbf{Supplementary Appendix}\\[4pt]
\large Social Vulnerability and Progression to Vision-Threatening Diabetic
Retinopathy: A County-Level Analysis of 3,119 United States Counties, 2021}
\date{}

\begin{document}
\maketitle

\noindent This appendix documents every hypothesis we tested, including those
that did not support the analysis reported in the main text. Each is presented
as: what is tested conceptually, how we tested it, what we found, and how we
interpret it.
...
composition. Overdispersion also fell sharply, from a dispersion parameter of
...
\tpart{(2) How we tested it.}
Five tests of increasing stringency.
...
\emph{2e. Program type.} Distance to the nearest university-based program and to
the nearest community-hospital program entered jointly, classified from program
names (98 university-based, 25 community/hospital). If the mechanism is
subspecialty depth rather than the presence of trainees, university proximity
should dominate.
...
($P = 0.06$). The two exposures correlate only $0.41$. The signs are opposite:
...
\emph{2d.} Capacity-weighted access $\beta = +0.010$, $P = 0.06$, wrong-signed.

\emph{2e.} University program distance $\beta = -0.028$ ($P < 0.001$);
community-hospital program distance $\beta = +0.000$ ($P = 0.99$). The two
distances correlate $0.61$, so this is a real distinction rather than
collinearity. Note this test runs on 1,333 counties, those having a
community-hospital program among their routed candidates, a subsample skewed
urban and eastern.

In the spatial model reported in the main text, academic drive time retains a
...
pattern is that proximity to specialist eye care, and especially to university
programs where subspecialty retinal diagnosis concentrates, predicts \emph{more}
diagnosed vision-threatening disease. This is difficult to read as academic
...
holding race fixed, with identical covariate sets, and tested the difference
with a two-sample $z$ statistic on the coefficient difference.
...
\midrule
40--64 years & 20.3\% & 0.150 & 0.0039 \\
65--84 years & 18.0\% & 0.142 & 0.0040 \\
\bottomrule
\end{tabular}
\end{center}
Difference $0.008$; $z = 1.42$; $P = 0.16$; ratio of odds ratios $1.01$. The
same pattern holds within each race stratum (main text, Table 2): 0.034 versus
0.032 among non-Hispanic White adults, 0.037 versus 0.035 among non-Hispanic
Black adults, 0.046 versus 0.037 among Hispanic adults.
